...
void LCD_init(uint8_t addr, uint8_t dataPin, uint8_t clockPin, uint8_t cols, uint8_t rows)
{
    ...
    LCD_rows = rows;
    I2C_init();
    vTaskDelay(200 / portTICK_PERIOD_MS);                                 // Initial 40 mSec delay

    // Reset the LCD controller
    LCD_writeNibble(LCD_FUNCTION_RESET, LCD_COMMAND);                   // First part of reset sequence
    vTaskDelay(20 / portTICK_PERIOD_MS);                                  // 4.1 mS delay (min)
    ...
    ets_delay_us(160);                                                   // 40 uS delay (min)
    ...
    vTaskDelay(4 / portTICK_PERIOD_MS);                                   // Clearing memory takes a bit longer
...
void LCD_home(void)
{
    ...
    vTaskDelay(4 / portTICK_PERIOD_MS);                                   // This command takes a while to complete
}

void LCD_clearScreen(void)
{
    LCD_writeByte(LCD_CLEAR, LCD_COMMAND);
    vTaskDelay(5 / portTICK_PERIOD_MS);                                   // This command takes a while to complete
}
